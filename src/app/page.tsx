...
import ContributionImpact from '@/components/ContributionImpact'
      ...
      {!authenticated ? (
        <>
          <button
            ...
            className="rounded-md bg-blue-600 px-4 py-2 font-semibold text-black hover:bg-blue-700"
          >
            Login / Sign Up
          </button>
        ...
        <div>
          ...
          shadow-sm border-b border-gray-200 py-3 px-8 flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="bg-red-500 hover:bg-red-600 text-white px-4 py-1.5 rounded-md text-sm font-medium shadow-sm"
              >
                Logout
              </button>
            </div>
          ...
          <div className="flex flex-col lg:flex-row items-stretch justify-between w-full max-w-7xl gap-6 px-6 mt-8">
            <div className="flex-[1.2] bg-white p-5 rounded-xl shadow-md text-gray-700 w-full max-w-6xl">
              <h1 className="text-2xl font-bold mb-4 text-center text-gray-800">
                Welcome back, {username}! 👋
              </h1>
              <div className="flex flex-col lg:flex-row gap-6 w-full max-w-7xl px-6">
                {/* TWO-COLUMN LAYOUT: HISTORY LEFT | OVERVIEW RIGHT */}
                <div className="flex flex-col md:flex-row gap-8 w-full">
                  {/* LEFT: Contribution History */}
                  <div className="flex-[1.1] bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                {showEdit && (
                  <div className="fixed inset-0 flex items-center justify-center bg-black/40">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm">
                      ...
                      {selection.type === 'fixed' && selection.value <= 0 && (
                        <p className="text-red-600 text-sm mb-2">
            ...
            <div className="w-full lg:w-[40%] bg-white p-6 rounded-xl shadow-md text-gray-700">
              <ContributionImpact
                ...
                ytdContributions={snapshot.ytdContributions ?? 0}
                contributionType={selection.type ?? 'percent'}
                contributionValue={selection.value ?? 0}
                onAccept={async (newValue, newType) => {
                  await fetch('/api/contribution', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      snapshot,
                      selection: { type: newType, value: newValue },
                    }),
                  })
                  fetchContribution()
                }}
